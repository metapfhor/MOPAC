...
C     DEFAULT VALUE RETURNED BECAUSE NO NUMERIC FIELD FOUND
   50 READA=0.D0
      RETURN
      END
      DOUBLE PRECISION FUNCTION READN(STRING,ISTART)
C     FORTRAN FUNCTION TO EXTRACT NUMBER FROM STRING
C
      CHARACTER STRING*(*)
      DOUBLE PRECISION DIGIT
...
***************************************
*       SUBROUNTINE MODIFIED TO RETURN ZERO IF A CHARACTER
*       IS FOUND BEFORE ANY NUMBERICAL DATA
***************************************
C       END LAURENT
C
C     DEFINE ASCII VALUES OF NUMERIC FIELD CHARACTERS
...
C       ACCOUNT FOR CONSECUTIVE DECIMAL POINTS (.)
         ...
         IF(N.NE.ISPACE.AND.IADD.EQ.0)GOTO 50
...
C     FOUND THE END OF THE NUMERIC FIELD (IT RUNS 'I' THRU 'J-1')
      ...
      N=N+INDEX(STRING(I:J-1),'D')
      IF(N.EQ.0)THEN
         READN=DIGIT(STRING(I:J-1),1)
      ELSE
         READN=DIGIT(STRING(:I+N-2),I)*1.D1**DIGIT(STRING(:J-1),I+N)
      ENDIF
      RETURN
C
C     DEFAULT VALUE RETURNED BECAUSE NO NUMERIC FIELD FOUND
