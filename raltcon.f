      ...
      INTEGER ICONXN(6,NUMATM),CONX(4),II,JJ
...
C     FIRST INFO ON THIS LINE IS A TRANSLATION
   40   CONX(1)=INT(TMPC(1))
      ...
      NVALUE=0
      II=0
      DO WHILE(II.LE.MAXCHAR/2)
        ISTART(II)=0
      END DO
      DO 70 I=1,MAXCHAR
         IF (LEADSP.AND.CHUNK(I:I).NE.SPACE) THEN
            NVALUE=NVALUE+1
            ISTART(NVALUE)=I
         END IF
         LEADSP=(CHUNK(I:I).EQ.SPACE)
   70 CONTINUE

      TMPC(7)=0
      DO 80 I=1,6
       TMPC(I)=0
       TMPC(I)=READN(CHUNK,ISTART(I))
       IF(TMPC(7).EQ.0.AND.TMPC(I).EQ.0)TMPC(7)=I
   80 CONTINUE

      IF(TMPC(7).EQ.2) THEN
       CALL ADDTRANS(CONX(1),INT(TMPC(2)),GETINTS(CHUNK(ISTART(4):)))
